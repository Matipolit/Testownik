<script lang="ts">
    ...
    let toggleAnswerCallback: (index: number) => boolean = $state(() => false);
    let ended: boolean = $state(false);
    let skipQuestionNum: number = $state(1);
    let curr_image: string | undefined = $state(undefined);
    let anyAnswerSelected: boolean = $state(false);

    function markAnswer(answerIndex: number): void {
        anyAnswerSelected = toggleAnswerCallback(answerIndex);
    }
    ...
    function handleKeydown(event: KeyboardEvent): void {
        ...
        } else if (key === "enter" && checked && !ended) {
            nextQuestion();
            event.preventDefault();
...
>
        ...
        >
            ...
            <div class="bg-gray-700 p-2 text-sm hidden md:block">
                ...
                <ul class="text-xs space-y-0.5">
                    <li>
                        <kbd class="bg-gray-600 px-1 rounded">A-Z</kbd> - Zaznacz
                        odpowiedź
                    </li>
                    <li>
                        <kbd class="bg-gray-600 px-1 rounded">Enter</kbd> - Sprawdź
                        / Następne
                    </li>
                </ul>
            </div>
            ...
            <div
                class="bg-gray-700 p-2 flex gap-4 items-center justify-between"
            >
                <div>
    ...
    >
        ...
        {:else}
            <div class="p-2">
                <h2 class="text-2xl">Test zakończony</h2>
                <button onclick={resetTest}><RotateCcw />Testuj ponownie</button
                >
                <div>
                    <h3 class="text-lg mt-4 mb-2">Podsumowanie:</h3>
                    <p>
                        Poprawne: <b class="text-green-300">{points}</b> |
                        Niepoprawne:
                        <b class="text-red-300">{negativePoints}</b>
                    </p>
                    <p>{(points / db.length) * 100} %</p>
                </div>
            </div>
        {/if}
    </div>
